...
def get_track_info(track_ids):
    ...
    for track_id in track_ids:
        result = db.session.query(Track.trackName, Track.artist).filter_by(songId=track_id).first()
...
def transfer_playlist(userID, playlistID):
    ...
    for track, artist in playlist_tracks.items():
        ...
        if not song:
            song = Track(trackName=official_name, artist=artist)
            db.session.add(song)
            db.session.commit()  
    ...
    return jsonify({"message": "Playlist transferred successfully!"}), 200